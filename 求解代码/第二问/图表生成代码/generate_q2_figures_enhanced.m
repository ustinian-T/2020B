...
title(layout, '第三关1024个情景下两类策略的财富差距', ...
    'FontSize', 16, 'FontWeight', 'bold', 'Color', navy);
subtitle(layout, '两条阶梯线之间的色带表示完全信息带来的财富提升', ...
...
title(ax, '第三关1024个情景的财富与后悔值流向', ...
    'FontSize', 16, 'FontWeight', 'bold', 'Color', navy);
subtitle(ax, '流带宽度表示情景数量  左右节点高度表示对应层级频数', ...
    'FontSize', 11, 'Color', ink);
...
title(layout, '第四关沙暴预算的稳健前沿与资源代价', ...
    'FontSize', 16, 'FontWeight', 'bold', 'Color', navy);
subtitle(layout, '全部结果直接读取模型输出  Γ7至Γ9仅作为扩展压力测试  安全下界不代表全局最优', ...
    'FontSize', 11, 'Color', ink);
...
title(layout, '第四关三种策略的三万条样本外证据', ...
    'FontSize', 16, 'FontWeight', 'bold', 'Color', navy);
subtitle(layout, '同一批Markov天气测试集  财富与后悔值分布只统计成功样本  参照上界不可直接执行', ...
    'FontSize', 11, 'Color', ink);
...
title(layout, '第四关沙暴扰动与天气机制的双重灵敏性复核', ...
    'FontSize', 16, 'FontWeight', 'bold', 'Color', navy);
subtitle(layout, '点面积随平均到达日增大  矩阵由题内30日序列估计并仅作为名义评价基准', ...
    'FontSize', 11, 'Color', ink);
