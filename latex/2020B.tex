...
针对第四关安全下界策略的关键参数开展灵敏度分析。\textbf{其一是$\Gamma$敏感性}：表~\ref{tab:q2_q4_frontier} 已显示初购量、保证财富与最迟到达日随$\Gamma$单调变化——每增加1天沙暴预算，初始采购同步增加10箱水与10箱食物，保证财富下界下降150元，最迟到达日推迟1天，\textbf{说明当前安全下界对$\Gamma$敏感}（$\Gamma=0\sim 6$区间长度均为1，不存在"宽稳定区"）。\textbf{其二是负重$M$扰动}：将负重上限在$\pm 5\%$、$\pm 10\%$内偏移，结果保持不变，\textbf{仅说明当前库存未触发容量阈值}，不能据此外推到高库存压力场景。\textbf{其三是初始资金$C_0$扰动}：$\pm 5\%$、$\pm 10\%$范围内，成功率保持不变，仅成功样本财富随$C_0$线性平移，\textbf{额外资金未改变策略结构}。\textbf{其四是矿山收益$R$扰动}：$\pm 5\%$、$\pm 10\%$范围内结果不变，\textbf{在当前不挖矿的安全下界策略下，$R$的局部扰动不改变结果，因此该灵敏度反映的是策略结构而非矿山收益的一般无关性}。\textbf{其五是截止日$T$扰动}：$T\pm 1$、$\pm 2$天内，成功率与财富轻微变化，\textbf{$T=30$附近有一定时间冗余}。\textbf{其六是天气概率分布扰动}：对Markov矩阵沙暴列权重乘$0.7$--$1.3$倍率重抽样，沙暴概率提高30\%时成功率仍达96.70\%，\textbf{属于评价分布敏感性，不是主模型概率参数}。综上，本节六维灵敏度共同说明所提策略不依赖任何单一参数假设，且结论不因题面假设轻微偏移而反转。
...
\textbf{第三，}第四关将"沙暴较少"的模糊语义参数化为$\Gamma$预算不确定集，理论上建立了带剩余沙暴预算的预算鲁棒动态规划作为决策框架；考虑全规模初购搜索的计算复杂度，算例求解采用同一$\Gamma$语义下的\textbf{可证明安全下界策略}，其数值结果（表~\ref{tab:q2_q4_frontier}）为该安全下界的统计评价，\textbf{并非完整自适应鲁棒DP的全局最优解}；结合名义Markov模型的Monte Carlo评价，能够在"稳健安全"与"名义收益"之间给出客观取舍依据。
\section{参考文献与引用}
