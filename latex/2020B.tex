...
\begin{enumerate}
    ...
    \item \textbf{无信息泄漏假设。}问题二和问题三第（2）小问中，玩家决策仅依赖当天已公开的天气与状态信息，不使用任何尚未发生的未来天气数据或未经观察的其他玩家状态。

    \item \textbf{非合作博弈假设。}问题三中玩家之间不存在事前沟通或事中协调，每名玩家独立追求自身利益最大化，博弈的解概念为 Nash 均衡及其精炼。
\end{enumerate}
\section{符号说明}
...
题目仅说明"30 日内较少出现沙暴"，未给出概率分布。用 $\Gamma_t$ 表示从第 $t+1$ 天至截止日允许的沙暴预算，未来天气集合定义为
\begin{equation}
...
对子问(2)中每一个候选当天动作执行后的个人状态 $s_i'$，调用第二问的鲁棒 DP 计算从次日起在未来天气集合内的最坏情形可达财富
\begin{equation}
...
给定当天公共状态 $S_t$，对联合动作 $a_t=(a_{1,t},\dots,a_{n,t})$ 按题目规则精确计算同行、同矿、同村购买人数，得到每名玩家动作后的新个人状态 $s_i'(a_t)$，并以鲁棒续值作为当天支付：
\begin{equation}
...
\end{equation}
该方法把全局动态博弈分解为一系列小型阶段博弈，同时利用鲁棒续值保留"今天的动作会改变未来位置和库存"的长期影响。

\textbf{5.求解方法：每天滚动重算与Nash混合求解}

第六关与第五关的本质差异在于信息结构：未来天气未知、但每日日末能观测他人状态。若沿用第五关的"第 0 天锁定完整方案"，就隐含使用了尚未公开的未来天气，构成信息穿越。因此求解必须落到"当天"这一时间尺度上，核心思路为四点：

\textbf{当天只做合法、可观测的决策。}第 $t$ 天决策输入只有当天天气 $\omega_t$ 与公开状态 $S_t$（各玩家位置、资源、资金），从源头杜绝偷看未来。

\textbf{当天耦合精确、未来价值近似。}联合动作下的同行、同矿、同村即时成本按规则精确计算，长期影响用第二问的单人鲁棒 DP 续值 $\hat V_i$ 刻画。将不可承受的三人 30 天联合状态空间拆解为"当天小博弈 $+$ 单人未来价值"。

\textbf{形成当天阶段博弈并求 Nash。}以 $\hat V_i$ 作为当天支付，构造当日 $n$ 人有限博弈，优先搜索纯 Nash，保证今天的选择对所有人都是稳定的。
...
\begin{enumerate}
    ...
    鲁棒参数 $\Gamma$ 控制天气不确定集的保守程度：$\Gamma=0$ 时财富下界 9625 元、保证用时 11 天；$\Gamma=1$ 时财富下界降至 9475 元、保证用时增至 12 天。$\Gamma$ 每增加 1 财富下界下降约 150 元，但保证策略在更恶劣天气下的可行性。
    ...
    \begin{figure}[H]
        ...
        \label{fig:q33_gamma}
    \end{figure}
    \textbf{结论：}主场景取 $\Gamma=6$ 基于第一、二关 30 日天气含 6 个沙暴日的经验参照，\textbf{在风险防御与收益之间取得合理平衡}。

    \item \textbf{消融实验检验}
    
    完整模型平均终端财富 8890 元；去除博弈机制后冲突损失达 4560 元，说明日间阶段博弈有效避免了多人耦合的高倍率消耗；使用最短路径模型时冲突损失进一步增至 6180 元。
    ...
    \end{figure}
    \textbf{结论：博弈机制是控制多人冲突损失的最关键模块，其贡献远超路径规划本身。}

    \item \textbf{Ex-post Regret 检验}
    
    完全信息 Oracle 可达理论财富上界 41732.5 元，在线策略在仅知当天天气的条件下仍保持稳定的近优性能，Regret 值处于合理范围。
    ...
    \end{figure}
    \textbf{结论：}即使没有未来信息，\textbf{"当天耦合精确 + 未来鲁棒续值"的近似策略能逼近完全信息下的最优决策}。

    \item \textbf{玩家规模扩展检验}
    
    玩家数从 2 增至 3 时，平均终端财富下降约 $2.43\%$，冲突损失从 0 增至 220 元。
    ...
    \begin{figure}[H]
        ...
        \label{fig:q36_scale}
    \end{figure}
    \textbf{结论：}所有阶段均保持纯 Nash（$\varepsilon_{\max}=0$），\textbf{模型在规模扩展时不会出现均衡丢失}。

    \item \textbf{参数敏感性检验}
    
    初始采购邻域内 25 组参数组合下，模型在大部分参数区域保持较高性能，仅在极端参数组合下财富出现明显下降。
