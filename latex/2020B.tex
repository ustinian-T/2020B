...
\section{问题分析}
        ...
        第二小问中，天气与未来竞争位置不定。考虑到完整三人Markov博弈，但不确定因素过多导致其笛卡尔积过大，因此转换方法选用局部耦合价值求解。将当天的同行、同矿、同村计算出来，并经单人鲁棒DP给出停留、挖矿、移动动作的未来价值。最后得到结果。
        根据这些值生成当日的小型阶段博弈求Nash均衡，只执行当天动作，次日重新求解。
\section{模型假设}

\begin{enumerate}
    \item \textbf{离散时间与整数资源假设。}游戏以天为最小时间单位，所有决策（移动、停留、挖矿、补给）均以天为周期执行；水和食物的计量单位为箱，采购与消耗量均为整数，不考虑连续数量下的部分消耗。

    \item \textbf{完全理性假设。}所有玩家均以终端可变现财富（到达时的现金加剩余资源半价回收）最大化为唯一目标，在决策中不包含风险偏好、合作倾向或利他行为等非理性因素。

    \item \textbf{无信息泄漏假设。}玩家决策仅依赖当天已公开的天气与状态信息，不使用任何尚未发生的未来天气数据或未经观察的其他玩家状态。
...
\end{equation}
    ...
    \hspace*{2em}为避免"先透支再到村庄补给"的非法行为，从非村庄节点移动至村庄当日的消耗由移动前库存满足，购买只作用于后续状态。
...
由表~\ref{tab:q2_q4_frontier}可知：\textbf{$\Gamma=6$为推荐锚点}，所有$\Gamma$共用同一8步最短路。$\Gamma$每增加1，初始采购同步增加10箱水、10箱食物，\textbf{保证财富下界下降150元，最迟到达日推迟1天}。需注意，本表为预算鲁棒模型的可证明安全下界，未包含村庄补给与矿山收益的全局最优鲁棒前沿。
...
图~\ref{fig:q2_strategy_evidence} 对比四种策略：\textbf{鲁棒$\Gamma=6$与高安全基线均接近全成功（98.84\%）}，均值财富分别为7489.81元和7264.81元；第一问固定方案与$\Gamma=2$分别出现43.13\%与33.62\%的失败区。固定方案成功样本均值虽高（8127元），但该条件均值排除了大量失败样本，不能单独作为策略优劣依据。\textbf{高安全基线与$\Gamma=6$成功率相同但均值低约225元}，证明非"买得越多越好"。
...
\textbf{6.天气概率分布扰动}：对Markov矩阵沙暴列权重乘$0.7$--$1.3$倍率重抽样，沙暴概率提高30\%时成功率仍达96.70\%，\textbf{属于评价分布敏感性，不是主模型概率参数}。
...
第五关 $n=2$，$T=10$，整个游戏期天气全部已知；每名玩家必须在第 0 天一次性确定完整方案并保持不变。该信息结构严格对应开环（open-loop）有限期完全信息非合作动态博弈：每个完整方案是一个纯策略，纯 Nash 均衡要求任一玩家在对手方案不变时没有盈利偏离。

\textbf{1.决策变量与完整策略}

玩家 $i$ 的纯策略 $\pi_i$ 由第 0 天起点采购 $(q^w_{i,0},q^f_{i,0})$ 与第 $1\sim T$ 天的完整行动计划组成，包括向相邻区域移动 $x_{i,t,u,v}$、普通停留 $s_{i,t}$、矿山挖矿 $z_{i,t,m}$ 以及村庄补给 $q^w_{i,t},q^f_{i,t}$。由于策略在第 0 天锁定，$\pi_i$ 一旦选择此后不可修改，这正是开环策略的定义。

\textbf{2.多人互动与状态转移}

设 $K^E_{t,u,v}=\sum_i x_{i,t,u,v}$ 为第 $t$ 天由 $u$ 行至 $v$ 的同行人数，$K^M_{t,m}=\sum_i z_{i,t,m}$ 为第 $t$ 天矿山 $m$ 的挖矿人数，$K^V_{t,v}=\sum_i\mathbf{1}(q^w_{i,t}+q^f_{i,t}>0,\ L_{i,t-1,v}=1)$ 为第 $t$ 天村庄 $v$ 发生购买的玩家数。动作消耗倍率
\begin{equation}
...
C_{i,t}&=C_{i,t-1}-\alpha_{i,t,v}\bigl(p_w q^w_{i,t}+p_f q^f_{i,t}\bigr)+\sum_m z_{i,t,m}\cdot\frac{R}{K^M_{t,m}},
\end{aligned}
\end{equation}
其中 $\alpha_{i,t,v}=1$（第 0 天起点）或 $2$（村庄且 $K^V=1$）或 $4$（村庄且 $K^V\ge2$）。同矿时消耗仍为 3 倍基础消耗，收益按 $R/K^M$ 分摊；同村且 $K^V\ge2$ 时价格为 4 倍基准价。

\textbf{3.目标函数与约束}

到达终点后剩余资源按基准价的一半退回。玩家 $i$ 最大化其终端可变现财富
\begin{equation}
...
\textbf{4.Nash 均衡条件与 $\varepsilon$ 检验}

若策略组合 $\pi^*=(\pi_1^*,\dots,\pi_n^*)$ 满足对每个玩家 $i$，在其他玩家方案不变时任何可行的单边替换都不能提高自身终端财富，则称其为纯策略 Nash 均衡：
\begin{equation}
U_i(\pi_i^*,\pi_{-i}^*)\ge U_i(\pi_i,\pi_{-i}^*),\quad \forall\pi_i\in\Pi_i,\ \forall i\in I.
\end{equation}
单边盈利偏离定义为 $\varepsilon_{i,t}=\max_{a_i\in A_i}\bigl[U_i(a_i,\pi_{-i}^*)-U_i(\pi_i^*,\pi_{-i}^*)\bigr]$，纯 Nash 要求 $\varepsilon=\max_{i,t}\varepsilon_{i,t}=0$。该条件比"冲突最少"或"总资金最大"更直接对应个人理性。

\textbf{5.求解方法：DP 与 Nash混合求解}


前面的状态转移方程已把多人耦合刻画为"同行倍率 $2K^E$、同矿收益 $R/K^M$、同村价格 $4$ 倍"三类时变参数。一旦其他玩家方案固定，这三类参数退化为已知常数，玩家 $i$ 的问题重归单人 DP，可用第一问的资源约束 DP 精确求全局最优响应 $BR_i(\pi_{-i})$。因此只需把 DP 改造成最佳响应 Oracle，围绕它组织均衡搜索。
...
默认参数 $n=2,\ T=10,\ R=200,\ M=1200,\ C_0=10000$ 下求得纯策略 Nash 均衡（$\varepsilon=0$），结果如表~\ref{tab:q5_result} 所示。
...
两人首日主动走不同方向，玩家 2 在第 2 个高温日选择停留以避免与玩家 1 形成高成本同向移动。规则审计分别完成 21 项与 28 项检查，违规数和最大守恒残差均为 0；固定对手后重新调用全局 DP 求 Best Response，两人的盈利偏离均为 0，直接确认 Nash 稳定性。
...
\end{figure}
如图~\ref{fig:q31_resource} 所示，两玩家水和食物库存均呈阶梯式下降：行走日消耗加速、停留日消耗减缓。玩家 1 因路线更短（第 3 天到达）总消耗量较小，但玩家 2 通过主动避让避免了同向移动的高倍率消耗，终端财富反而略高于玩家 1。水资源在两人策略中均比食物更快逼近下限，是制约持续行动的关键因素。

\subsubsection{第三问(2)：第六关——滚动时域鲁棒阶段博弈模型}
...
\hspace*{2em}日末观察被题目明确允许，故第 $t$ 天决策时公共状态为
\begin{equation}
S_t=\bigl\{(v_{i,t-1},W_{i,t-1},F_{i,t-1},C_{i,t-1})_{i=1}^n,\ \omega_t,\ t\bigr\}.
\end{equation}
每名玩家当天合法动作集 $A_i(S_t,\omega_t)$ 由地图相邻性、天气、当前位置、资源与资金约束自动生成，包括向相邻区域移动、普通停留、矿山挖矿与村庄补给等组合。沙暴日删除所有移动动作。
...
\end{equation}
$\Gamma$ 不是"真实概率"，而是"较少沙暴"的数学化情景变量。第一、二关公开的 30 日天气序列含 6 个沙暴日，故 $\Gamma=6$ 仅作为同题环境的经验参照点。

\textbf{3.单人鲁棒续值}

\hspace*{2em}对子问(2)中每一个候选当天动作执行后的个人状态 $s_i'$，调用第二问的鲁棒 DP 计算从次日起在未来天气集合内的最坏情形可达财富
\begin{equation}
\hat{V}_i(t{+}1,s_i';\,\Gamma_t)=\max_{\pi_i}\min_{\omega\in\Omega_t(\Gamma_t)}U_i(\pi_i;\,\omega\mid s_i').
\end{equation}
该续值只评价个人未来，不需猜测其他玩家未来 30 天的完整轨迹；其他玩家真正产生的耦合将在之后每天重新进入"当前阶段博弈"。

\textbf{4.当天阶段博弈与局部耦合价值分解}

\hspace*{2em}给定当天公共状态 $S_t$，对联合动作 $a_t=(a_{1,t},\dots,a_{n,t})$ 按题目规则精确计算同行、同矿、同村购买人数，得到每名玩家动作后的新个人状态 $s_i'(a_t)$，并以鲁棒续值作为当天支付：
\begin{equation}
Q_i(S_t,a_t)=\hat{V}_i(t{+}1,s_i'(a_t);\Gamma_t').
\end{equation}
当天 Nash 均衡满足
\begin{equation}
Q_i(S_t,a_i^*,a_{-i}^*)\ge Q_i(S_t,a_i,a_{-i}^*),\quad \forall a_i\in A_i(S_t,\omega_t).
\end{equation}
\hspace*{2em}该方法把全局动态博弈分解为一系列小型阶段博弈，同时利用鲁棒续值保留"今天的动作会改变未来位置和库存"的长期影响。

\textbf{5.求解方法：每天滚动重算与Nash混合求解}

\hspace*{2em}第六关与第五关的本质差异在于信息结构：未来天气未知、但每日日末能观测他人状态。若沿用第五关的"第 0 天锁定完整方案"，就隐含使用了尚未公开的未来天气，构成信息穿越。因此求解必须落到"当天"这一时间尺度上，核心思路为四点：

\hspace*{2em}\textbf{当天只做合法、可观测的决策。}第 $t$ 天决策输入只有当天天气 $\omega_t$ 与公开状态 $S_t$（各玩家位置、资源、资金），从源头杜绝偷看未来。
...
\textbf{只执行当天动作、次日重算。}当天均衡只执行一天，日末记录公开观测，次日基于新状态重新建模。未来冲突在真正发生前逐日进入当前阶段，而非提前假设对手的固定轨迹。
...
默认参数 $n=3,\ T=30,\ R=1000,\ M=1200,\ C_0=10000$，取 $\Gamma=6$（经验参考）的主场景下三名玩家在第 14 天全部到达终点，结果如表~\ref{tab:q6_result} 所示。
...
30 天所有已执行阶段均为纯 Nash，$\varepsilon_{\max}=0$。规则审计共 363 项，违规 0、残差 0；冲突等值损失仅来自同向移动，共 220 元（同矿、同村损失均为 0）；未来天气信息泄漏反事实检验 $t=1,\dots,30$ 共 30 次全部通过。Bootstrap 经验重采样（30 条样本，随机种子 42）：29/30 成功，成功率 $96.67\%$，成功样本终端财富均值 $7957.7$ 元、标准差 $491.7$ 元、中位数 $7920$ 元、95\% CI $[7854.4,8061.0]$ 元，所有样本 $\varepsilon_{\max}=0$。
